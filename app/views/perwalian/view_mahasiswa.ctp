...
<table cellpadding="0" cellspacing="0" class="Design">
	...
	<tbody>
		...
		<tr onClick="sembunyi('daftar_nilai'+<?=$mahasiswa['Tmahasiswa']['NIM']?>)">
			...
			<td>				
				<?php echo $ajax -> link('Lihat Rekap SKS',""	,array ("url"=>'/perwalian/history_mahasiswa/'.$mahasiswa['Tmahasiswa']['NIM'],'update'=> "daftar_nilai".$mahasiswa['Tmahasiswa']['NIM']));?>
				<?php if(!empty($mahasiswa['FormStudi'])):?> |				
				<?php echo $ajax -> link('Lihat FRS',""	,array ("url"=>'/perwalian/frs_mahasiswa/'.$mahasiswa['Tmahasiswa']['NIM'],'update'=> "daftar_nilai".$mahasiswa['Tmahasiswa']['NIM']));?> |
				<?php echo $ajax -> link('Setuju',""	,array ("url"=>'/perwalian/setuju/'.$mahasiswa['Tmahasiswa']['NIM'],'update'=> "daftar_nilai".$mahasiswa['Tmahasiswa']['NIM']));?> |
				<?php echo $ajax -> link('Tolak',""	,array ("url"=>'/perwalian/tolak/'.$mahasiswa['Tmahasiswa']['NIM'],'update'=> "daftar_nilai".$mahasiswa['Tmahasiswa']['NIM']));?> |
				<?php echo $ajax -> link('Isi Notes',""	,array ("url"=>'/perwalian/isi_notes/'.$mahasiswa['Tmahasiswa']['NIM'],'update'=> "daftar_nilai".$mahasiswa['Tmahasiswa']['NIM']));?>
				<?php endif;?>
		</td>
			</td>	
		</tr>
